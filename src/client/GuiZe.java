...
public class GuiZe {
	...
	public boolean canMove(int startI,int startJ,int endI,int endJ,String name){
		...
		else if(name.equals("炮")||name.equals("砲")){this.pao(maxI,minI,maxJ,minJ,startI,startJ,endI,endJ);}
	...
	private void pao(int maxI, int minI, int maxJ, int minJ, int startI,
		...
		if(maxI==minI){//如果在一条竖线上
			if(qizi[endI][endJ]!=null){
				...
				for (int i =minJ+1; i < maxJ; i++) {//遍历这条线
					if(qizi[minI][i]!=null){
						count++;
					}
				}
		...
		}else if(maxJ!=minJ&&maxI!=minI){//如果不在同一条线上
			canMove=false;
		}
