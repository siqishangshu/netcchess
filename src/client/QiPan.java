...
public class QiPan extends JPanel implements MouseListener {
	...
	public void paint(Graphics g1){
		...
		g.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);//打开抭锯齿
		Color c=g.getColor();//获得画笔颜色
		g.setColor(XiangQi.bgcolor);//将画笔设为背景颜色
			...
			for (int j = 0; j < 10; j++) {//绘制棋子
				if(qizi[i][j]!=null){
					if(this.qizi[i][j].getFocus()!=false){//是否被选中
						...
						g.setColor(XiangQi.focuschar);//字体颜色
					}else{
						g.fillOval(110+i*60-25,80+j*60-25, 50, 50);//绘制该棋子
	...
	public void showShuYu(String name,int startI,int startJ,int endI,int endJ){
		...
		if(this.xq.caiPan){
			...
			if(name.equals("将")||name.equals("砲")||name.equals("卒")||name.equals("车")){
